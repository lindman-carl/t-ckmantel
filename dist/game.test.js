...
import { games, playersInGame, gameCreate, gameAddPlayer, gameRemovePlayer, gameVote, gameEliminatePlayer, gameStart, } from "./game.js";
describe("gameCreate", () => {
    beforeEach(() => {
...
describe("gameAddPlayer", () => {
    ...
    test("should not add a player to a game if the game does not exist", () => {
        ...
        const updatedGame = gameAddPlayer(gameIdThatDoesntExist, playerId, playerName);
        // expect the game to be null
    ...
    test("should be able to add multiple players to a game", () => {
        ...
        const hostName = "hostName";
        const numPlayers = 10;
        const playerIds = Array.from({ length: numPlayers }, (_, i) => `playerId${i}`);
        const playerNames = Array.from({ length: numPlayers }, (_, i) => `playerName${i}`);
        // create a game
        gameCreate(gameId, hostId, hostName);
    ...
    test("should be able to add multiple players to multiple games", () => {
        ...
        const hostNames = Array.from({ length: numGames }, (_, i) => `hostName${i}`);
        const numPlayers = 5;
        const playerIds = Array.from({ length: numPlayers }, (_, i) => `playerId${i}`);
        ...
        gameIds.forEach((gameId) => {
            playerIds.forEach((playerId, i) => {
                ...
                const updatedGame = gameAddPlayer(gameId, playerIdWithGameId, playerName);
                if (updatedGame === null) {
                    fail("gameAddPlayer returned null");
                }
                // get the game from the games map
...
describe("gameVote", () => {
    ...
    test("should not eliminate a player if there is a tie", () => {
        ...
        const numPlayersLeftInGame = Object.values(game.players).reduce((acc, curr) => acc + (curr.inGame ? 1 : 0), 0);
    ...
    test("should end game when the last undercover is voted out", () => {
        ...
        const undercoverPlayer = Object.entries(game.players).find(([_, player]) => {
        ...
        const nonUndercoverPlayers = Object.keys(game.players).filter((playerId) => {
    ...
    test("should end game when there is no longer a majority of non-undercover players", () => {
        ...
        // count players .inGame
        let numPlayersLeftInGame = Object.values(game.players).reduce((acc, curr) => acc + (curr.inGame ? 1 : 0), 0);
        ...
        // count players .inGame
        numPlayersLeftInGame = Object.values(game.players).reduce((acc, curr) => acc + (curr.inGame ? 1 : 0), 0);
    ...
    test("should give correct scores with many players", () => {
        ...
        // get players who are not undercover
        let nonUndercoverPlayers = Object.keys(game.players).filter((playerId) => {
            return (!undercoverPlayers.find((id) => id === playerId) &&
        ...
        expect(game.round).toBe(1);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[0]]).toBe(10);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[1]]).toBe(10);
        //
        // ROUND 2
        //
        // make a tie
        ...
        expect(numPlayersLeftInGame).toBe(10);
        expect(game.round).toBe(2);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[0]]).toBe(20);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[1]]).toBe(20);
        //
        // ROUND 3
        //
        // vote for someone who is not undercover
        ...
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[0]]).toBe(30);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[1]]).toBe(30);
        //
        // ROUND 4
        //
        // vote for someone who is not undercover
        ...
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[0]]).toBe(40);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[1]]).toBe(40);
        //
        // ROUND 5
        //
        ...
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[0]]).toBe(40);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[1]]).toBe(50);
        //
        // ROUND 6
        //
        // vote for someone who is not undercover
        ...
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[0]]).toBe(40);
        expect(game.accumulatedScoreForUndercoverPlayers[undercoverPlayers[1]]).toBe(60);
        //
        // ROUND 7
        //
